...
#if defined(NDEBUG)
# error "DigitalDirham cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x150680289afce6f6328dd6528e87fd4824645c7a43346bce4c967ce99eaa18b3");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // DigitalDirham: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "DigitalDirham Signed Message:\n";
...
{
    // DigitalDirham: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 100000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 100000); // DigitalDirham: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 60; // DigitalDirham: readjust difficulty every 10 minutes
static const int64 nTargetSpacing = 1 * 30; // DigitalDirham: one block each 30 sec
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // DigitalDirham: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // DigitalDirham: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x150680289afce6f6328dd6528e87fd4824645c7a43346bce4c967ce99eaa18b3");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "28 de Nov 2014, Presentacion del adelanto de la pelicula The Force Awakens";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1417195782;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1799018;
        ...
        assert(block.hashMerkleRoot == uint256("0xf2ee7197b2989962331492ea78a1026b53720a18dabc1813d8faeddafcb20dd1"));
...
unsigned char pchMessageStart[4] = { 0xc3, 0xd2, 0xd1, 0xbd }; // DigitalDirham: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DigitalDirhamMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DigitalDirhamMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DigitalDirhamMiner(CWallet *pwallet)
{
    printf("DigitalDirhamMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("digitaldirham-miner");
    ...
    try { loop {
        ...
        printf("Running DigitalDirhamMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DigitalDirhamMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DigitalDirhamMiner, pwallet));
}
